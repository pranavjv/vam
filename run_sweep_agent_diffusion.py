...
from sweep_vradam_diffusion import train_model, create_vradam_sweep_config, create_adam_sweep_config, create_sgd_sweep_config, create_rmsprop_sweep_config

def run_sweep_agent(optimizer_name, count=10):
    """
    Create and run a new W&B sweep agent for a specific optimizer on diffusion model.
    ...
    if optimizer_name == 'VRADAM':
        print(f"Creating new VRADAM sweep for diffusion model on MNIST")
        sweep_config = create_vradam_sweep_config()
        project_name = "VRADAM-optimization-Diffusion-MNIST"
    elif optimizer_name == 'ADAM':
        print(f"Creating new ADAM sweep for diffusion model on MNIST")
        sweep_config = create_adam_sweep_config()
        project_name = "ADAM-optimization-Diffusion-MNIST"
    elif optimizer_name == 'SGD':
        print(f"Creating new SGD sweep for diffusion model on MNIST")
        sweep_config = create_sgd_sweep_config()
        project_name = "SGD-optimization-Diffusion-MNIST"
    elif optimizer_name == 'RMSPROP':
        print(f"Creating new RMSPROP sweep for diffusion model on MNIST")
        sweep_config = create_rmsprop_sweep_config()
        project_name = "RMSPROP-optimization-Diffusion-MNIST"
    else:
    ...
    config_path = f"{results_dir}/sweep_config_Diffusion_MNIST.json"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run a W&B sweep agent for optimizer comparison on diffusion models")
