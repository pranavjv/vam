import argparse
import wandb
import json
import os
# Import the specific config creators and the generalized train_model
from sweep_vadam_rl import train_model, create_vadam_sweep_config, create_adam_sweep_config

def run_sweep_agent(optimizer_name, count=10):
    """
    Create and run a new W&B sweep agent for a specific optimizer on RL task.
    
    Args:
        optimizer_name: Name of the optimizer ('VADAM' or 'ADAM')
        count: Number of runs to perform
    """
    # Ensure wandb is logged in
    wandb.login()
    
    # Select the appropriate config function and set project name
    if optimizer_name == 'VADAM':
        print(f"Creating new VADAM sweep for RL on HalfCheetah")
        sweep_config = create_vadam_sweep_config()
        project_name = "VADAM-optimization-RL-HalfCheetah"
    elif optimizer_name == 'ADAM':
        print(f"Creating new ADAM sweep for RL on HalfCheetah")
        sweep_config = create_adam_sweep_config()
        project_name = "ADAM-optimization-RL-HalfCheetah"
    else:
        raise ValueError(f"Unsupported optimizer_name: {optimizer_name}. Choose 'VADAM' or 'ADAM'.")

    # Initialize the sweep
    sweep_id = wandb.sweep(
        sweep_config, 
        project=project_name,
    )
    
    # Save sweep configuration and ID locally
    results_dir = f"sweep_results_{optimizer_name}"
    os.makedirs(results_dir, exist_ok=True)
    config_path = f"{results_dir}/sweep_config_RL_HalfCheetah.json"
    with open(config_path, 'w') as f:
        json.dump(sweep_config, f, indent=2)
        
    print(f"Created new sweep with ID: {sweep_id} in project '{project_name}'")
    print(f"Sweep config saved to: {config_path}")
    
    # Run the sweep agent
    print(f"Starting sweep agent for {count} experiments (Sweep ID: {sweep_id}) ...")
    wandb.agent(sweep_id, function=train_model, count=count)
    print(f"Sweep agent finished for sweep ID: {sweep_id}")
    
    return sweep_id

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run a W&B sweep agent for VADAM or ADAM optimization on RL tasks")
    parser.add_argument("--optimizer_name", type=str, required=True, choices=["VADAM", "ADAM"],
                      help="Optimizer to run the sweep for ('VADAM' or 'ADAM')")
    parser.add_argument("--count", type=int, default=10, help="Number of runs to perform")
    
    args = parser.parse_args()
    
    # Run the sweep agent
    sweep_id = run_sweep_agent(
        optimizer_name=args.optimizer_name,
        count=args.count
    ) 